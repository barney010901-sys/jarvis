...
_SYSTEM_PROMPT = (
    ...
    "available tools, respond with ONLY a JSON array of 1-5 steps, each "
    'shaped {"description": string, "tool_name": string|null}. Set '
    '"tool_name" only when the step should actually invoke one of the '
    "listed tools, by exact name. No prose, no markdown code fences — the "
    "entire response must be valid JSON and nothing else."
)


class ClaudePlanner(PlannerInterface):
    def __init__(self, router: ModelRouter, tool_names: list[str], *, fallback: PlannerInterface | None = None) -> None:
        self._router = router
    ...
    async def plan(self, task_id: str, request: str) -> Plan:
        try:
            prompt = (
                f"Available tools: {', '.join(self._tool_names) or '(none)'}\n\n"
...
def _parse_steps(text: str) -> list[PlanStep]:
    ...
    return [PlanStep(description=str(item["description"]), tool_name=item.get("tool_name")) for item in data]
